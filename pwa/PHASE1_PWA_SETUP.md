...
cd purehub-pwa
npm install
npm install react-router-dom zustand idb lucide-react vite-plugin-pwa tailwindcss @tailwindcss/vite i18next react-i18next i18next-browser-languagedetector
npm run dev
```
...
- Route-level i18n is prepared for `en`, `vi`, and `zh`, with locale-prefixed URLs such as `/en/compass` and `/vi/la-ban`.
