...
declare global { interface Window { turnstile?: { render: (element: HTMLElement, options: { sitekey: string; action: string; callback: (token: string) => void; 'expired-callback': () => void }) => void } } }
...
export function MinigamePage() {
  ...
  const [ticket, setTicket] = useState(() => params.get('ticket') || sessionStorage.getItem('purehub:minigame-ticket') || '')
  ...
  useEffect(() => {
    ...
    const render = () => window.turnstile?.render(turnstileRef.current!, { sitekey: campaign.turnstile_site_key, action: 'minigame-entry', callback: setTurnstileToken, 'expired-callback': () => setTurnstileToken('') })
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { const entry = await api<{ entry_id: string; picked_number: string }>('/entries', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ticket, picked_number: number, phone_number: phone, gmail, turnstile_token: turnstileToken }) }); sessionStorage.removeItem('purehub:minigame-ticket'); setTicket(''); setNotice(`Đã chốt số ${entry.picked_number}. Mã lượt chơi: ${entry.entry_id}`) }
  ...
  return <main data-minigame-reveal className="mx-auto min-h-screen max-w-2xl px-4 py-8 text-slate-900 dark:text-white">
    <section className="overflow-hidden rounded-[28px] bg-gradient-to-br from-violet-700 via-fuchsia-600 to-amber-500 p-6 text-white shadow-xl"><Gift className="size-10" /><p className="mt-5 text-xs font-black uppercase tracking-[.18em] text-white/75">PureHub beta tester</p><h1 className="mt-2 text-3xl font-black leading-tight">Dự đoán 2 số<br />Nhận thẻ điện thoại mỗi ngày</h1><p className="mt-3 leading-7 text-white/90">Chọn số từ 00–99 trước 18:00. 5 bạn đoán đúng sớm nhất nhận thẻ điện thoại mệnh giá {prize}đ.</p>{!ticket && <Link to="/vi/download" className="mt-5 flex min-h-12 items-center justify-center rounded-2xl bg-emerald-400 px-4 text-center font-black text-emerald-950 shadow-lg transition hover:bg-emerald-300">📲 Tải PureHub cho Android để chốt số</Link>}</section>
    ...
    <section className="mt-5 rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex items-start gap-3"><CircleCheck className="mt-0.5 size-6 shrink-0 text-emerald-500" /><div><h2 className="font-bold">Kết quả ngày {campaign?.day ?? '…'}</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{settled ? `Số trúng: ${result.winning_number} · ${result.winner_count} người nhận thẻ` : 'Hệ thống bắt đầu đối chiếu từ 18:30 và tự kiểm tra lại nếu nguồn kết quả chậm.'}</p></div></div>{settled ? <WinnerList winners={result.winners} /> : <div className="mt-4 rounded-2xl border border-dashed border-slate-300 p-4 dark:border-slate-700"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Ví dụ hiển thị sau khi công bố</p><p className="mt-1 text-sm"><b>Số trúng: 45</b> · 5 người nhận thẻ điện thoại 10.000đ</p><WinnerList winners={exampleWinners} /><p className="mt-3 text-xs text-slate-500">SĐT và Gmail luôn được che một phần; chỉ người quản trị thấy thông tin đầy đủ để nạp thẻ.</p></div>}</section>
    {showAndroidPrompt && <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/65 p-5"><section className="relative w-full max-w-sm rounded-[28px] bg-white p-6 text-slate-900 shadow-2xl"><button onClick={() => setShowAndroidPrompt(false)} className="absolute right-3 top-3 grid size-9 place-items-center rounded-full bg-slate-100 p-0 text-slate-600"><X className="size-5" /></button><Smartphone className="size-10 text-emerald-600" /><h2 className="mt-4 text-xl font-black">Vui lòng mở App PureHub trên Android để tham gia nhận lộc!</h2><p className="mt-3 text-sm leading-6 text-slate-600">Cài đặt app trên điện thoại Android, vào <b>Cộng đồng → Minigame</b>. App sẽ mở trang chốt số hợp lệ cho bạn.</p><Link to="/vi/download" className="mt-5 flex min-h-12 items-center justify-center rounded-2xl bg-emerald-500 px-4 text-center font-black text-emerald-950">📲 Tải PureHub cho Android ngay</Link><button onClick={() => setShowAndroidPrompt(false)} className="mt-3 w-full border-0 bg-transparent text-slate-500">Để xem kết quả thôi</button></section></div>}
  </main>
}
