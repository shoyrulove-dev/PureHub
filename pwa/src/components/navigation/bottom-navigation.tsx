...
export function BottomNavigation() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-3xl border-t border-slate-200/80 bg-white/94 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 backdrop-blur">
        ...
        {TAB_ITEMS.map((tab) => (
          <li key={tab.id}>
            <NavLink
              ...
              className={() =>
                [
                  'flex min-h-[56px] flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-medium transition',
                  activeTabId === tab.id
