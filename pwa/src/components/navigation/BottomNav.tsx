...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto w-full max-w-4xl px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] xl:absolute">
      <ul className="app-surface grid grid-cols-4 gap-1 rounded-[20px] p-1.5 backdrop-blur-xl">
        {items.map(({ label, icon: Icon, path, active }) => (
          <li key={label}>
            <NavLink
              to={path}
              className={[
                'flex min-h-[56px] flex-col items-center justify-center gap-1 rounded-[15px] px-2 text-[11px] font-semibold transition',
                active
