...
export default function SpeakerCleanerFlagship() {
  ...
  const [soundCheck, setSoundCheck] = useState<'idle' | 'before' | 'after'>('idle')
  ...
  return <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
    ...
    <div className="space-y-5 p-4 sm:p-5">
      ...
      <section className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
        <div className="flex items-start gap-3"><Ear className="mt-0.5 size-5 shrink-0 text-cyan-700 dark:text-cyan-300" /><div><h3 className="text-sm font-black">Before / after sound check</h3><p className="mt-1 text-xs leading-5 text-slate-500">Play the same familiar audio outside PureHub and compare clarity at a comfortable volume. PureHub never records your speaker.</p></div></div>
        <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => setSoundCheck('before')} className={`min-h-11 rounded-xl border text-xs font-black ${soundCheck === 'before' ? 'border-cyan-500 bg-cyan-50 text-cyan-900 dark:bg-cyan-950 dark:text-cyan-100' : 'border-slate-200 dark:border-slate-700'}`}>Before checked</button><button disabled={!completed} onClick={() => setSoundCheck('after')} className={`min-h-11 rounded-xl border text-xs font-black disabled:cursor-not-allowed disabled:opacity-45 ${soundCheck === 'after' ? 'border-emerald-500 bg-emerald-50 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-100' : 'border-slate-200 dark:border-slate-700'}`}>After checked</button></div>
        {soundCheck === 'after' ? <p role="status" className="mt-3 flex items-center gap-2 text-xs font-bold text-emerald-700 dark:text-emerald-300"><CheckCircle2 className="size-4" />Comparison complete. Stop here if clarity is restored.</p> : null}
      </section>
