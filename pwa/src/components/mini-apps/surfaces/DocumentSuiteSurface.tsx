...
type Page = { id: string; file: File; url: string; rotation: number; fit: 'contain' | 'cover' }
...
export default function DocumentSuiteSurface() {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    setPages((current) => [...current, ...accepted.map((file) => ({ id: crypto.randomUUID(), file, url: URL.createObjectURL(file), rotation: 0, fit: 'contain' as const }))])
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      for (let index = 0; index < pages.length; index += 1) {
        ...
        context.translate(canvas.width / 2, canvas.height / 2); context.rotate(page.rotation * Math.PI / 180); context.drawImage(image, -image.naturalWidth / 2, -image.naturalHeight / 2)
  ...
  return <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {pages.length ? <div className="space-y-2">{pages.map((page, index) => <article key={page.id} className="flex items-center gap-3 rounded-2xl border border-slate-200 p-2.5 dark:border-slate-700"><img src={page.url} alt={`Page ${index + 1}`} className="size-16 rounded-xl bg-slate-100 object-cover dark:bg-slate-950" /><div className="min-w-0 flex-1"><strong className="block truncate text-sm">Page {index + 1} · {page.file.name}</strong><div className="mt-2 flex gap-1"><button title="Move up" onClick={() => move(index, -1)} className="grid size-8 place-items-center rounded-lg bg-slate-100 dark:bg-slate-800"><ArrowUp className="size-3.5" /></button><button title="Move down" onClick={() => move(index, 1)} className="grid size-8 place-items-center rounded-lg bg-slate-100 dark:bg-slate-800"><ArrowDown className="size-3.5" /></button><button onClick={() => update(page.id, { rotation: (page.rotation + 90) % 360 })} className="rounded-lg bg-slate-100 px-2 text-xs font-bold dark:bg-slate-800">Rotate {page.rotation}°</button><button onClick={() => update(page.id, { fit: page.fit === 'contain' ? 'cover' : 'contain' })} className="rounded-lg bg-slate-100 px-2 text-xs font-bold dark:bg-slate-800">{page.fit}</button></div></div><button title="Remove page" onClick={() => remove(page.id)} className="grid size-9 place-items-center rounded-xl text-rose-600 hover:bg-rose-50"><Trash2 className="size-4" /></button></article>)}</div> : <div className="grid min-h-56 place-items-center rounded-[22px] border border-dashed border-slate-300 bg-slate-50 text-center dark:border-slate-700 dark:bg-slate-950"><div><FileImage className="mx-auto size-12 text-violet-500" /><strong className="mt-3 block">Your document starts here</strong><p className="mt-1 text-sm text-slate-500">Up to 20 local image pages.</p></div></div>}
