...
The current preview is PureHub 1.0.0-beta.40. Document Suite adds a private PDF toolbox for merging PDFs, extracting page ranges, producing smaller flattened copies, and adding a clearly labelled visual signature on Android and the PWA. Android uses its local PDF renderer to create safer flattened outputs without adding another network-capable SDK; the PWA uses a lazy-loaded, MIT-licensed PDF library and performs every operation in the browser. Visual signatures are not certificate-backed digital signatures, and the UI states this explicitly. The standard flavor remains offline-first while retaining Internet access for explicitly connected workflows such as the minigame and update checks. The F-Droid flavor remains strictly offline-only and replaces ML Kit OCR with Tesseract. Signed Android artifacts are published through GitHub Releases after CI, scanner checks, and physical-device verification. Each Android release provides:
