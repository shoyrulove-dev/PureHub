...
The current release candidate is PureHub 1.0.0-beta.44. It fixes OCR review rotation leaking into the next capture, compacts retained page previews for safer 20-page documents, releases failed/deleted page bitmaps, and prunes image-backed documents that fall out of local history. It retains beta.43's four-corner perspective correction, complete local document library, and TXT/searchable-PDF export, plus beta.42's social queue reliability, accurate Deep Cleaner reporting, and security hardening. The standard flavor remains offline-first while retaining Internet access only for explicitly connected workflows such as the Minigame and update checks. The F-Droid flavor remains strictly offline-only and replaces ML Kit OCR with Tesseract. Signed Android artifacts are published through GitHub Releases after CI, scanner checks, and physical-device verification. Each Android release provides:
