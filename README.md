...
The current preview is PureHub 1.0.0-beta.38. Money Studio now tracks income, expenses, monthly balance, categories, and wallets without an account, with a non-destructive Room migration for existing ledgers. Document Suite adds one-tap batch cleanup and keeps gallery editing available without camera permission. Android Screen Recorder can optionally include microphone narration with explicit permission. PWA top-level routes now load on demand, reducing the initial production chunk below the previous warning threshold. The standard flavor remains offline-first while retaining Internet access for explicitly connected workflows such as the minigame and update checks. The F-Droid flavor remains strictly offline-only and replaces ML Kit OCR with Tesseract. Signed Android artifacts are published through GitHub Releases after CI, scanner checks, and physical-device verification. Each Android release provides:
