...
The current release candidate is PureHub 1.0.0-beta.45. It improves Vietnamese receipt parsing for OCR output that uses grouped thousands and recognizes a degraded `đ` currency symbol as `d`, while preventing trailing disclaimer text from shifting amount-to-label pairing. It retains beta.44's safer 20-page OCR memory handling and local-history cleanup, beta.43's perspective correction and document export, and beta.42's social queue reliability, accurate Deep Cleaner reporting, and security hardening. The standard flavor remains offline-first while retaining Internet access only for explicitly connected workflows such as the Minigame and update checks. The F-Droid flavor remains strictly offline-only and replaces ML Kit OCR with Tesseract. Signed Android artifacts are published through GitHub Releases after CI, scanner checks, and physical-device verification. Each Android release provides:
