...
The current preview is PureHub 1.0.0-beta.39. Document Suite now detects document edges locally for conservative one-tap framing, while OCR Studio accepts multi-image batches, lets users review each page, detects receipt totals, and can save a confirmed receipt directly to the offline Money Studio ledger. The same batch OCR, page review, receipt handoff, and local auto-framing workflows are available in the PWA. No document image or recognized text is uploaded by these features. The standard flavor remains offline-first while retaining Internet access for explicitly connected workflows such as the minigame and update checks. The F-Droid flavor remains strictly offline-only and replaces ML Kit OCR with Tesseract. Signed Android artifacts are published through GitHub Releases after CI, scanner checks, and physical-device verification. Each Android release provides:
