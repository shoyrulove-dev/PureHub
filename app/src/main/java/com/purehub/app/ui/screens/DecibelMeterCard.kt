        ...
        Column(
            ...
            FlowRow(
                ...
                Button(
                    ...
                    LocalizedText(if (uiState.isRunning) "Monitoring" else "Start")
                }
                Button(onClick = viewModel::stop) {
                    LocalizedText("Stop")
                }
                Button(onClick = viewModel::resetPeak) {
                    LocalizedText("Reset Peak")
                }
            }
            ...
            LocalizedText("Calibration offset ${if (uiState.calibrationOffset >= 0) "+" else ""}${uiState.calibrationOffset.toInt()} dB")
            Slider(value = uiState.calibrationOffset, onValueChange = viewModel::setCalibrationOffset, valueRange = -20f..20f, steps = 39)
            LocalizedText(uiState.accuracyWarning, style = MaterialTheme.typography.bodySmall, color = colorScheme.tertiary)
            PrivacyReceipt(
            ...
            if (mode == SuiteMode.PRO && uiState.sessionHistory.isNotEmpty()) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    LocalizedText("Recent sessions", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold)
                    OutlinedButton(onClick = viewModel::clearHistory) { LocalizedText("Clear") }
                }
                uiState.sessionHistory.take(5).forEach { session ->
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            LocalizedText("Min ${session.minimumDecibel.toInt()} · Avg ${session.averageDecibel.toInt()} · Max ${session.maximumDecibel.toInt()} dB")
                            LocalizedText("${session.points.size} one-second samples", style = MaterialTheme.typography.bodySmall, color = colorScheme.onSurfaceVariant)
