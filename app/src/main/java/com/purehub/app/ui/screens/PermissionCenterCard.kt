...
fun PermissionCenterCard() {
    ...
    val permissions = listOf(
        ...
        PermissionExplanation(Manifest.permission.RECORD_AUDIO, "Microphone", "Decibel Meter and optional Screen Recorder narration. Speaker Cleaner plays tones and does not listen."),
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            permissions.forEach { item ->
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.Top) {
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        LocalizedText("${item.label} · ${if (granted) "Allowed" else "Not allowed"}", style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.Medium)
                        LocalizedText(item.reason, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
            }
            OutlinedButton(
            ...
            ) { Icon(Icons.AutoMirrored.Rounded.OpenInNew, null); LocalizedText(" Open Android app settings") }
        }
    }
}
